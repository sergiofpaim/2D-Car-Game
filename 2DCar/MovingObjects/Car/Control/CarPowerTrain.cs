...
namespace MovingObjects.Car
{
    public class CarPowerTrain
    {
        ...
        private static readonly List<Gear> gears = new()
        {
            ...
            new() { Code = "2", Ratio = 0.9F, RPMVariation = 1900 },
            new() { Code = "3", Ratio = 1.3F, RPMVariation = 1200 },
            new() { Code = "4", Ratio = 2.2F, RPMVariation = 700 },
        ...
        public float Accelerate()
        {
            ...
            {
                RPM -= gears[currentGear].RPMVariation * BREAK_FACTOR * Scheduler.LoopInterval / 1000;
                if (RPM < IDLE_RPM)
                { 
                    PutInNeutralPause();
                    GearUp();
            ...
            {
                RPM += gears[currentGear].RPMVariation * Scheduler.LoopInterval / 1000;
                if (RPM > MAX_RPM)
                    GearUp();
            }
        ...
        private bool IsInNeutralPause()
        {
            if (idlePauseInMs > 0)
            {
                idlePauseInMs -= Scheduler.LoopInterval;
                return true;
            }
            else 
                return false;
        ...
        public float Idle()
        {
            ...
            if (currentGear < NEUTRAL_GEAR)
            {
                RPM -= gears[currentGear].RPMVariation * IDLE_FACTOR * Scheduler.LoopInterval / 1000;
                if (RPM <= IDLE_RPM)
                {
                    RPM = IDLE_RPM;
                    GearUp();
